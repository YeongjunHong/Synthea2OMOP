...
class SyntheaToOmop5:
    ...
    def patientsToOmop(self, df, personmap, person_id, location_id):
        ...
        personmap = personmap._append(personappend)
    ...
    # def proceduresToOmop(df, srctosrcvm, procedure_occurrence_id, personmap, visitmap):
    #     procedure_occurrence = pd.DataFrame()

    #     for index, row in df.iterrows():
    #         omop_procedure = {
    #             'procedure_occurrence_id': procedure_occurrence_id,
    #             'person_id': personmap[row['PATIENT']],
    #             'procedure_concept_id': srctosrcvm[row['CODE']],
    #             'procedure_date': row['START'],  # Assuming 'START' is the start date of the procedure
    #             'visit_occurrence_id': visitmap[row['ENCOUNTERCLASS']],
    #             # Add other necessary OMOP fields here
    #         }
            
    #         procedure_occurrence = procedure_occurrence.append(omop_procedure, ignore_index=True)
    #         procedure_occurrence_id += 1
        
    #     return procedure_occurrence, procedure_occurrence_id
    # In SyntheaToOmop5.py
    def proceduresToOmop(self, df, srctostdvm, procedure_occurrence_id, personmap, visitmap):
        ...
        srctostdvm_filtered = srctostdvm[(srctostdvm["target_domain_id"]=='Procedure') & 
                                        (srctostdvm["target_vocabulary_id"]=='SNOMED') & 
                                        (srctostdvm["target_standard_concept"]=='S') & 
                                        (srctostdvm["target_invalid_reason"].isnull())]
        
        concept_df = pd.merge(df[['CODE']], srctostdvm_filtered[['source_code','target_concept_id']], 
        ...
        procedure_occurrence['procedure_date'] = pd.to_datetime(df['START']).dt.date
        procedure_occurrence['procedure_datetime'] = pd.to_datetime(df['START'])
        procedure_occurrence['procedure_end_date'] = pd.to_datetime(df['STOP']).dt.date
        procedure_occurrence['procedure_end_datetime'] = pd.to_datetime(df['STOP'])
        procedure_occurrence['procedure_type_concept_id'] = '38000275'  # EHR
        procedure_occurrence['modifier_concept_id'] = '0'
        procedure_occurrence['quantity'] = 1
        procedure_occurrence['provider_id'] = None  # You may need to add this if you have provider information
        procedure_occurrence['visit_occurrence_id'] = df['visit_occurrence_id']
        procedure_occurrence['visit_detail_id'] = None  # You may need to add this if you have visit detail information
        procedure_occurrence['procedure_source_value'] = df['CODE'].astype(str)
        procedure_occurrence['procedure_source_concept_id'] = df['CODE']
        procedure_occurrence['modifier_source_value'] = None  # You may need to add this if you have modifier information
    ...
    #     # Map columns to OMOP vocabulary
    #     procedure_occurrence['procedure_occurrence_id'] = df['Id'].map(srctosrcvm)
    #     procedure_occurrence['person_id'] = df['PATIENT'].map(personmap)
    #     procedure_occurrence['procedure_concept_id'] = df['CODE'].map(srctosrcvm)
    #     procedure_occurrence['procedure_date'] = pd.to_datetime(df['START']).dt.date  # Use 'START' column for procedure date
    #     procedure_occurrence['procedure_datetime'] = pd.to_datetime(df['START'])
    #     procedure_occurrence['procedure_type_concept_id'] = 38000253  # Assuming a constant concept ID for procedure type

    #     # Handle missing values in reason code and description
    #     procedure_occurrence['modifier_concept_id'] = np.nan
    #     procedure_occurrence['quantity'] = np.nan
    #     procedure_occurrence['provider_id'] = df['PROVIDER'].map(srctosrcvm)
    #     procedure_occurrence['visit_occurrence_id'] = df.apply(lambda x: findVisit(visit, x['PATIENT'], x['START'], x['STOP']), axis=1)
    ...
    def encountersToOmop(self, df, observation_period_id, visit_occurrence_id, personmap, visitmap):
        ...
        observation_period = pd.DataFrame(columns=self.model_schema['observation_period'].keys())
        observation_period['observation_period_id'] = observation_period_id + np.arange(len(observation_tmp))
        observation_period['person_id'] = observation_tmp['person_id']
        observation_period['observation_period_start_date'] = observation_tmp['first']
        observation_period['observation_period_end_date'] = observation_tmp['last']
        observation_period['period_type_concept_id'] = '44814724'

        # Check if 'observationtmp' column exists before dropping
        if 'observationtmp' in observation_period.columns:
            observation_period = observation_period.drop('observationtmp', axis=1)
        else:
            print("'observationtmp' column not found in observation_period DataFrame.")

        observation_period_id += len(observation_period)
        ...
        # Ensure visitmap is initialized correctly as a DataFrame
        if visitmap is None:
        ...
        return observation_period, visit_occurrence, visit_occurrence_id + len(visit_occurrence), observation_period_id, visitmap
